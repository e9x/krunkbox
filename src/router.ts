...
async function routerTpLinkArcherAx3000(
  ...
  if (doFreeKeys && pathname === "/slavelabor" && req.method === "GET") {
    ...
    if (!seenCCChecksums.has(checksum)) {
      ...
      if (!comparison || comparison.breakdown.nodeTypeDistribution < 0.99) {
        ...
        console.log(
          `krunkbox: CC node type distribution ${(comparison.breakdown.nodeTypeDistribution * 100).toFixed(1)}% >= 99%, skipping webhook.`,
        );
